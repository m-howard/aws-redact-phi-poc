...
def main(event, context):
  for record in event['Records']:
    ...
    tmp_converted_path = '/tmp/converted-{}'.format(tmpkey)
    ...
    # Convert file to pdf
    convert(tmp_download_path, tmp_converted_path)

    # Split PDF and upload new docs to staging area
    document = PdfFileReader(open(tmp_converted_path, "rb"))
    for i in range(document.numPages):
      ...
      new_key = key.replace('.docx', '/{}.pdf'.format(i))
